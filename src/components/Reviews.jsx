...
class Reviews extends React.Component {
    ...
    return (
      <Grid container item id="reviews-list" direction="column">
            ...
            >
              ...
              <Grid item>
                {body.length > 250 ? (
                  <Typography
                    variant="body1"
                    onClick={() => this.toggleShowMore(review_id)}
                  >
              ...
              <Grid className="review-footer" item>
                <Typography variant="body2">
                  ...
                  <Link
                    underline="always"
                    onClick={() => this.markHelpful(review_id)}
                  >
                    Yes
                  </Link>
                  ...
                  <Link
                    underline="always"
                    onClick={() => this.reportReview(review_id)}
                  >
                    Report
                  </Link>
                </Typography>
        ...
        <Grid id="reviews-buttons" container item>
          {this.props.reviews.length > this.state.showReviews ? (
            <Button
              id="more-reviews"
              xs={4}
              disableElevation
  ...
  markHelpful(review_id) {
    ...
    fetch(`/reviews/helpful/${review_id}/`, { method: "PUT" }).then(() => {
      this.props.update();
      this.setState({
        markedHelpful: { ...this.state.markedHelpful, [review_id]: true },
      });
    });
  }

  reportReview(review_id) {
    fetch(`/reviews/report/${review_id}/`, { method: "PUT" }).then(() =>
      this.props.update()
    );
  }
